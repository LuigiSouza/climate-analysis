...
def get_from_api(lat, lon):
    ...
    url = "https://api.openweathermap.org/data/2.5/onecall?lat=%s&lon=%s&exclude=current,hourly,minutely,alerts&appid=%s&units=metric" % (lat, lon, api_key)
...
def main():      
    ...
    create_file(str(date.today())+".csv", data_extracted)
